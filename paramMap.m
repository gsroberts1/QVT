...
% Last Modified by GUIDE v2.5 19-Jun-2020 11:43:09
...
global branchList Planes hfull p branchLabeled Ntxt nframes fov res VENC
...
%%% Data Loading
if  indx > 1  %if a pre-processed case is selected
    set(handles.TextUpdate,'String','Loading Preprocessed Data'); drawnow;
    load([directory filesep fn{indx}],'data_struct') %load data_struct
    ...
    res = data_struct.res; %image matrix size (pixels)
    fov = data_struct.fov; %image field of view (cm)
    ...
    [nframes,res,fov,timeres,VENC,area_vol,diam_vol,flowPerHeartCycle_vol, ...
    ...
    saveFile = fullfile(directory,['pcviprData_' saveState '.mat']);
    save(saveFile,'data_struct','Vel_Time_Res')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global Planes branchList segment directory fov res
set(handles.TextUpdate,'String','Opening Visual Tool'); drawnow;
fourDvis(Planes,branchList,segment,directory,fov,res);
uiwait;
...
global segment1 MAGcrossection vTimeFrameave fig
...
% Plot flow waveform
if length(pside)==2
    plot(5:5:100,smooth(flowPulsatile_vol(pindex,:)),...
        'k',5:5:100,smooth(flowPulsatile_vol(pside(1),:)),...
        'r',5:5:100,smooth(flowPulsatile_vol(pside(2),:)),...
        'r','LineWidth',5,'Parent',hfull.pfwaveform)
elseif length(pside)==3
    plot(5:5:100,smooth(flowPulsatile_vol(pindex,:)),...
        'k',5:5:100,smooth(flowPulsatile_vol(pside(1),:)),...
        'r',5:5:100,smooth(flowPulsatile_vol(pside(2),:)),...
        'r',5:5:100,smooth(flowPulsatile_vol(pside(3),:)),...
        'b','LineWidth',5,'Parent',hfull.pfwaveform)
else
    plot(5:5:100,smooth(flowPulsatile_vol(pindex,:)),...
        'k',5:5:100,smooth(flowPulsatile_vol(pside(1),:)),...
        'r',5:5:100,smooth(flowPulsatile_vol(pside(2),:)),...
        'r',5:5:100,smooth(flowPulsatile_vol(pside(3),:)),...
        'b',5:5:100,smooth(flowPulsatile_vol(pside(4),:)),...
        'b','LineWidth',5,'Parent',hfull.pfwaveform)
end
set(get(hfull.pfwaveform,'XLabel'),'String','Cardiac Time (%)','FontSize',12)
...
if branchLabeled ~= bnum
    ...
    Ntxt = text(branchActual(textint+1,2),branchActual(textint+1,1), ...
        branchActual(textint+1,3),numString_val,'Color','w','FontSize',14,...
